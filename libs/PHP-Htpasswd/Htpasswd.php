...
class Htpasswd {
    ...
    public function updateUser($username, $password, $encType = self::ENCTYPE_CRYPT)
    {
        ...
        $this->_users[$username] = self::encryptPassword($password, $encType);
        $this->_saveFile();

        return true;
    }

    public static function encryptPassword($password, $encType=self::ENCTYPE_CRYPT) {
        if ($encType == self::ENCTYPE_CRYPT) {
            ...
            $cryptPass = self::cryptApr1Md5($password);
            
        } elseif ($encType == self::ENCTYPE_SHA1) {
            $hash      = base64_encode(sha1($password, true));
            $cryptPass = '{SHA}' . $hash;
            
        } else { throw new Exception('Invalid encryption type'); }
